...
@ID_item VARCHAR(12),
@ID_mazmorra VARCHAR(5),
@drop_rate INT -- Agregamos este par�metro
AS
BEGIN
...
@ID_item VARCHAR(12),
@ID_mazmorra VARCHAR(5),
@nuevo_drop_rate INT -- Agregamos el par�metro que realmente queremos cambiar
AS
BEGIN
...
DECLARE @ID_Gremio INT = 1001; -- Cambiar por el que quieran
...
SELECT i1.tipo, i1.ID_item, i1.valor
FROM item i1
WHERE valor = (
    SELECT MAX(valor)
    FROM item i2
